...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def legalNeighbors: List[(Block, Move)] = for((x,y) <- neighbors if(x.isLegal)) yield (x,y)
     ...
     */
    def isLegal: Boolean = {
      if(isStanding && terrain(b1)) true
      else if(isStanding && !terrain(b1)) false
      else if(!isStanding && terrain(b1) && terrain(b2)) true
      else false
    }
  }
}
