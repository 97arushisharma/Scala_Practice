...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = if(b.isStanding && b.b1 == goal && b.b2 == goal) true else false
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = b.legalNeighbors.map({ case (x, y) => (x, y :: history) }).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filter({ case (x,y) => !explored.contains(x)})
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter({ case (x,y) => done(x)})
