...
trait Huffman extends HuffmanInterface {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case x :: xtail => (List(makeCodeTree(trees(0),trees(1))) ++ trees.tail.tail).sortBy(t => weight(t))
      //tree.sortBy(t => weight(t))(Ordering[Int].reverse)
      //tree.sortWith((t1, t2) => weight(t1) < weight(t2))
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    for((x,y) <- table if(x==char) ) yield y.head
  }
