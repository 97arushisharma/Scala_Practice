...
import scala.collection.{IterableOnce, SortedMap}
...
object Anagrams extends AnagramsInterface {
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = for ((ch, tm) <- x if (y.toMap.get(ch) match {
    case Some(s) => if (tm - s != 0) true else false
    case None => true
  })) yield (ch, y.toMap.get(ch) match {
    case Some(s) => tm - s
    case None => tm
  })

  //if y.toMap.contains(ch)
